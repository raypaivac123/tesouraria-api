...
export default function UniformePandeiro() {
  const [itens, setItens] = useState<ItemPandeiro[]>([]);
  const [congregacoes, setCongregacoes] = useState<Congregacao[]>([]);
  const [busca, setBusca] = useState("");
  const [congregacaoFiltro, setCongregacaoFiltro] = useState("");
  const [statusFiltro, setStatusFiltro] = useState("");
  const [editando, setEditando] = useState<ItemPandeiro | null>(null);
  ...
  async function carregar() {
    try {
      ...
      console.error("Erro ao carregar uniformes pandeiro", error);
    }
  }
  ...
  function abrirEdicao(item: ItemPandeiro) {
    setEditando(item);
    setForm({
  ...
  async function salvarEdicao(e: React.FormEvent) {
    ...
    try {
      await api.put(`/uniforme-pandeiro/${editando.id}`, {
        ...form,
      ...
      console.error("Erro ao atualizar uniforme pandeiro", error);
      alert(mensagemErroApi(error, "atualizar uniforme"));
    }
  }

  async function excluirItem(item: ItemPandeiro) {
    if (!window.confirm(`Deseja excluir o registro de "${item.nomeMulher}"?`)) return;

    try {
      await api.delete(`/uniforme-pandeiro/${item.id}`);
      await carregar();
      alert("Registro excluído com sucesso");
    } catch (error) {
      console.error("Erro ao excluir uniforme pandeiro", error);
  ...
  return (
    <Layout>
      <div className="page-header">
        <h2><i className="bi bi-music-note-beamed" aria-hidden="true"></i> Uniforme do Pandeiro</h2>
        <p>Controle de pagamentos do uniforme do pandeiro</p>
      </div>
      ...
      <div className="page-actions">
        ...
        <Link className="btn btn-primary" to="/novo-uniforme-pandeiro">
      ...
      {editando && (
        <div className="card" style={{ marginBottom: 20 }}>
          <div className="card-title">Editar Uniforme Pandeiro</div>
      ...
      <div className="card">
        <div className="table-wrapper">
          <table>
            <thead><tr><th>Nome</th><th>Congregação</th><th>Valor Total</th><th>Total Pago</th><th>Pendente</th><th>Status</th><th>Ações</th></tr></thead>
            <tbody>
        ...
        {itensFiltrados.length === 0 && <div className="empty-state"><div className="icon"><i className="bi bi-music-note-beamed" aria-hidden="true"></i></div><p>Nenhum registro encontrado.</p></div>}
      </div>
    </Layout>
  );
}
